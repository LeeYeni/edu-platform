...
export default function QuizPlay() {
  ...
  if (!roomCode) {
    return <p className="text-center text-red-500">방 코드가 존재하지 않습니다.</p>;
  }
  ...
  // ✅ 반 정보 일치 여부 검사
  const roomParts = roomCode.split("-");
  const userParts = user.userId.split("-");

  // roomCode: t-학교코드-학년-반-문제번호
  // userId:   s-학교코드-학년-반-(번호)
  const isSameClass =
    roomParts[1] === userParts[0] && // 학교코드
    roomParts[2] === userParts[1] && // 학년
    roomParts[3] === userParts[2];   // 반


  useEffect(() => {
    const user = JSON.parse(localStorage.getItem("user") || "{}");
    axios.get(`${BASE_URL}/api/quiz/result/exists`, {
      params: {
        userId: user.userId,
        questionId: roomCode,
      }
  ...
  useEffect(() => {
    if (roomCode) {
      axios.get(`${BASE_URL}/api/quiz/play/${roomCode}`)
        .then((res) => {
  ...
  const handleSubmit = () => {
    ...
    // s-인 경우에는 update 지원
      if (roomCode.startsWith("s-")) {
        const url = alreadySolved ? `${BASE_URL}/api/quiz/updateResult` : `${BASE_URL}/api/quiz/saveResult`;
      ...
      // t-인 경우는 저장만 1회 허용
        ...
        axios.post(`${BASE_URL}/api/quiz/saveResult`, payload)
          .then(() => {
            setQuizCompleted(true);
          })
          .catch((err) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* 잘못된 방 코드 */}
        {(!roomCode || (!roomCode.startsWith("s-") && !roomCode.startsWith("t-"))) && (
        ...
        {/* 우리 반이 아닐 때 */}
        {!(!roomCode || (!roomCode.startsWith("s-") && !roomCode.startsWith("t-"))) && !isSameClass && (
          <p className="text-red-600 font-bold text-center mt-10 text-xl">
            우리반이 아닙니다.
          </p>
        )}
