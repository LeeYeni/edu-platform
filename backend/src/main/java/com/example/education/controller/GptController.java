...
public class GptController {
    ...
    private static final String PROMPT_PREFIX =
            "아래의 대단원, 중단원, 소단원에 해당하는 초등학교 수학 문제를 생성해줘.\n" +
                    ...
                    "중요: 해설(explanation)은 명확하고 직관적으로 작성해야 합니다. '799가 아니라 799입니다'처럼 모순되거나 불필요하게 중복된 문장을 금지합니다.\n" +
                    "해설 작성 시 다음 3단계 구조를 반드시 따르세요:\n" +
                    "[문제 풀이 → 정답 도출 → 정답 근거 요약] 의 3단계로 깔끔하게 설명합니다.\n" +
                    "강제 조건: 'answer'로 지정한 보기는 반드시 해설에서 정답으로 설명해야 합니다. 예를 들어 answer가 'a'면, 해설에는 'a가 정답인 이유'를 써야 하고, 'a는 오답입니다' 같은 문장은 금지입니다.\n" +
                    "강제 조건: 문제, 보기(options), 정답(answer), 해설(explanation)은 하나의 세트로 일관성 있게 작성해야 한다.\n" +
                    "문제를 생성할 때 다음 흐름을 반드시 지켜라:\n" +
                    "① 문제(text) 생성 → ② 보기(options) 생성 → ③ 정답(answer) 선택 → ④ 선택한 정답(answer)의 id를 기반으로 해설(explanation) 작성.\n" +
                    "해설 작성 시 반드시 'answer'로 설정한 id와 text를 참조하여, answer가 지정한 선택지를 정확히 해설해야 한다.\n" +
                    "answer와 explanation이 불일치할 경우, 해당 문제는 무효 처리한다.\n" +
                    ...
                    "- '복수 선택형 문제'는 절대 출제하지 않는다. (ex: \"모두 고르세요\" 형태 금지)\n" +
                    "- 항상 문제 지문은 단일 정답 상황을 가정하여 작성하며, 복수 정답 가능성(single/multiple possibility)이 의심될 경우 해당 문제는 생성하지 않는다.\n" +
                    "\n" +
                    "※ 요약:\n" +
